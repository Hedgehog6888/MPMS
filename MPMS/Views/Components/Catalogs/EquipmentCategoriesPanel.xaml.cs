...
public partial class EquipmentCategoriesPanel : UserControl
{
    ...
    public EquipmentCategoriesPanel()
    {
        InitializeComponent();
    }
    ...
    private void ClearSearch_Click(object sender, RoutedEventArgs e)
    {
        if (DataContext is CatalogsViewModel vm)
            vm.EquipmentCategorySearchText = string.Empty;
    }

    private void AddCategory_Click(object sender, RoutedEventArgs e)
    {
        if (DataContext is CatalogsViewModel vm)
            vm.AddEquipmentCategoryCommand.Execute(null);
    }

    private void CategoryRow_Click(object sender, RoutedEventArgs e)
    {
        if (sender is Border border && border.DataContext is LocalEquipmentCategory cat)
        {
            if (DataContext is CatalogsViewModel vm)
                vm.ViewEquipmentCategoryCommand.Execute(cat);
        }
    }
}
