...
public partial class ProjectsViewModel : ViewModelBase, ILoadable
{
    ...
    private async Task LoadInternalAsync(LocalDbContext db, IQueryable<LocalProject> query, CancellationToken ct)
    {
        ...
        {
            var managerData = await db.Users
                .Where(u => managerIds.Contains(u.Id))
                .Select(u => new { u.Id, u.Name, u.AvatarData, u.AvatarPath })
                .ToDictionaryAsync(u => u.Id, ct);
            foreach (var p in list)
            {
                if (managerData.TryGetValue(p.ManagerId, out var md))
                {
                    p.ManagerName = md.Name;
                    var data = md.AvatarData;
                    if ((data is null || data.Length == 0) && !string.IsNullOrWhiteSpace(md.AvatarPath))
                    {
                        var fromFile = AvatarHelper.FileToBytes(md.AvatarPath);
                        if (fromFile is { Length: > 0 }) data = fromFile;
                    }
                    p.ManagerAvatarData = data;
                    p.ManagerAvatarPath = md.AvatarPath;
                }
            }
        }
